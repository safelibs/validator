...
if [[ -z "$config" || -z "$results_root" || -z "$site_root" ]]; then
  echo "usage: verify-site.sh --config <manifest> [--tests-root <dir>] --results-root <dir> [--artifacts-root <dir>] [--proof-path <path>] --site-root <dir>" >&2
  exit 1
fi

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)

python3 - "$repo_root" "$config" "$tests_root" "$results_root" "$site_root" "$artifacts_root" "$proof_path" <<'PY'
...
tests_root_arg = sys.argv[3]
results_root = Path(sys.argv[4])
site_root = Path(sys.argv[5])
artifacts_root_arg = sys.argv[6]
proof_path_arg = sys.argv[7]
...
if proof_file.get("proof_version") != 2:
    fail("proof_version must be 2")
if proof_file.get("mode") != "original":
    fail("proof mode must be original")
...
filtered_results = [result for result in all_results if str(result["library"]) in set(included_libraries)]
expected_keys = {
...
actual_keys = {(str(result["library"]), str(result["testcase_id"])) for result in filtered_results}
...
normalize = lambda rows: sorted(
    ...
    key=lambda row: (row["library"], row["testcase_id"]),
)
...
for row in site_rows:
    ...
    log_target = validate_relative_artifact_path(
        row["log_path"],
        field_name="log_path",
        source=f"site-data row {row['library']}/{row['testcase_id']}",
    )
...
for library_entry in site_proof["libraries"]:
    for case_entry in library_entry["cases"]:
        cast_path = case_entry.get("cast_path")
        cast_href = case_entry.get("cast_href")
        if cast_path is None:
            if cast_href is not None:
                fail(f"unexpected proof cast_href for {library_entry['library']}/{case_entry['testcase_id']}")
            continue
        if not cast_href:
            fail(f"missing proof cast_href for {library_entry['library']}/{case_entry['testcase_id']}")
        cast_target = validate_relative_artifact_path(
            cast_path,
            field_name="cast_path",
            source=f"proof row {library_entry['library']}/{case_entry['testcase_id']}",
        )
        assert cast_target is not None
        resolved_cast_href = resolve_site_href(
            cast_href,
        ...
        assert resolved_cast_href is not None
        if resolved_cast_href != cast_target:
            fail(f"proof cast_href does not match cast_path for {library_entry['library']}/{case_entry['testcase_id']}")
...
html_rows = set(re.findall(r'data-library="([^"]+)" data-testcase="([^"]+)"', html_text))
expected_html_rows = {(row["library"], row["testcase_id"]) for row in expected_rows}
...
for library_entry in proof_file.get("libraries", []):
    ...
    for case_entry in library_entry.get("cases", []):
        marker = f'data-proof-library="{library}" data-proof-testcase="{case_entry["testcase_id"]}"'
        if marker not in html_text:
