#!/usr/bin/env bash
# @testcase: usage-curl-include-custom-header-response
# @title: curl include custom response header
# @description: Requests with curl -i and verifies a custom server response header X-Validator-Tenth is included alongside the body.
# @timeout: 180
# @tags: usage, curl, headers
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-include-custom-header-response"
tmpdir=$(mktemp -d)
