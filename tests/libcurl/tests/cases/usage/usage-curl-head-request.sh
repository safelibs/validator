#!/usr/bin/env bash
# @testcase: usage-curl-head-request
# @title: curl reads HTTP headers
# @description: Performs a HEAD request against a local HTTP server and checks the status header.
# @timeout: 120
# @tags: usage, http
# @client: curl
...
case_id="usage-curl-head-request"
tmpdir=$(mktemp -d)
