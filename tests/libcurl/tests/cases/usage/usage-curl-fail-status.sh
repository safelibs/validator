#!/usr/bin/env bash
# @testcase: usage-curl-fail-status
# @title: curl fails on missing HTTP path
# @description: Requires curl --fail to reject a missing path from the local HTTP server.
# @timeout: 120
# @tags: usage, http
# @client: curl
...
case_id="usage-curl-fail-status"
tmpdir=$(mktemp -d)
