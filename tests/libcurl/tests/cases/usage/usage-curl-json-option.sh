#!/usr/bin/env bash
# @testcase: usage-curl-json-option
# @title: curl JSON convenience option
# @description: Posts JSON with curl --json and verifies the echoed content type and payload.
# @timeout: 180
...
case_id="usage-curl-json-option"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
