#!/usr/bin/env bash
# @testcase: usage-curl-json-header
# @title: curl sends JSON content type
# @description: Posts JSON with curl and verifies the local handler sees the content type and body.
# @timeout: 180
# @tags: usage, http, json
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-json-header"
tmpdir=$(mktemp -d)
