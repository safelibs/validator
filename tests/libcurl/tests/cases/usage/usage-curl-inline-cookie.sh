#!/usr/bin/env bash
# @testcase: usage-curl-inline-cookie
# @title: curl inline cookie
# @description: Sends an inline Cookie header with curl and verifies the server receives the expected cookie pair.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-inline-cookie"
tmpdir=$(mktemp -d)
