#!/usr/bin/env bash
# @testcase: usage-curl-basic-auth
# @title: curl sends basic auth
# @description: Uses curl basic authentication against a local HTTP handler and verifies authorization.
# @timeout: 180
# @tags: usage, http, auth
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-basic-auth"
tmpdir=$(mktemp -d)
