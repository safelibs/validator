#!/usr/bin/env bash
# @testcase: usage-curl-follow-redirect
# @title: curl follows redirect
# @description: Follows a local HTTP redirect with curl and verifies the final response body.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-follow-redirect"
tmpdir=$(mktemp -d)
