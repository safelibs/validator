#!/usr/bin/env bash
# @testcase: usage-curl-writeout-size-download
# @title: curl writeout size download
# @description: Downloads a fixed-size response and verifies the size_download writeout reports the exact byte count.
# @timeout: 180
...
case_id="usage-curl-writeout-size-download"
tmpdir=$(mktemp -d)
