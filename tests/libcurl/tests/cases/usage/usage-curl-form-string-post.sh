#!/usr/bin/env bash
# @testcase: usage-curl-form-string-post
# @title: curl form-string post
# @description: Posts multipart form data with curl --form-string and verifies the encoded field payload.
# @timeout: 180
...
case_id="usage-curl-form-string-post"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
