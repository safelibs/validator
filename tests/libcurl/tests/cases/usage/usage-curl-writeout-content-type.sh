#!/usr/bin/env bash
# @testcase: usage-curl-writeout-content-type
# @title: curl write-out content type
# @description: Uses curl write-out variables to capture the response content type from a local server.
# @timeout: 180
...
case_id="usage-curl-writeout-content-type"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
