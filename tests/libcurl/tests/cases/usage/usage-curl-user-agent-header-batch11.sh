#!/usr/bin/env bash
# @testcase: usage-curl-user-agent-header-batch11
# @title: curl user agent header
# @description: Sends a custom User-Agent header with curl to a loopback server.
# @timeout: 180
...
case_id="usage-curl-user-agent-header-batch11"
tmpdir=$(mktemp -d)
