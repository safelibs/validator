#!/usr/bin/env bash
# @testcase: usage-curl-cookie-file-send
# @title: curl cookie file send
# @description: Loads a Netscape cookie jar with curl and verifies the sent Cookie header on the server.
# @timeout: 180
...
case_id="usage-curl-cookie-file-send"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
