#!/usr/bin/env bash
# @testcase: usage-curl-put-payload-echo
# @title: curl put payload echo
# @description: Uploads a file with curl -T using HTTP PUT and verifies the server response echoes the uploaded body.
# @timeout: 180
# @tags: usage, curl, upload
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-put-payload-echo"
tmpdir=$(mktemp -d)
