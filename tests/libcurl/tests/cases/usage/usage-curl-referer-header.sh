#!/usr/bin/env bash
# @testcase: usage-curl-referer-header
# @title: curl referer header
# @description: Sends a Referer header with curl and verifies the server receives the configured origin URL.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-referer-header"
tmpdir=$(mktemp -d)
