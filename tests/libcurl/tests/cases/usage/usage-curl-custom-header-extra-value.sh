#!/usr/bin/env bash
# @testcase: usage-curl-custom-header-extra-value
# @title: curl sends custom header value
# @description: Sends a custom HTTP header with curl and verifies the echoed multi-word header value.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-custom-header-extra-value"
tmpdir=$(mktemp -d)
