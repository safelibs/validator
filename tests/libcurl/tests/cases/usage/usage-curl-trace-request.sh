#!/usr/bin/env bash
# @testcase: usage-curl-trace-request
# @title: curl TRACE request
# @description: Sends an HTTP TRACE request with curl and verifies the reflected request line.
# @timeout: 180
...
case_id="usage-curl-trace-request"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
