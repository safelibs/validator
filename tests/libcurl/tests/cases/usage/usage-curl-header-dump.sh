#!/usr/bin/env bash
# @testcase: usage-curl-header-dump
# @title: curl dumps headers
# @description: Writes HTTP response headers to a file with curl and checks the response metadata.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-header-dump"
tmpdir=$(mktemp -d)
