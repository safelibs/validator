#!/usr/bin/env bash
# @testcase: usage-curl-silent-fail-missing
# @title: curl silent fail missing
# @description: Confirms curl -f exits non-zero when the server returns 404 for a missing resource.
# @timeout: 180
# @tags: usage, curl, error
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-silent-fail-missing"
tmpdir=$(mktemp -d)
