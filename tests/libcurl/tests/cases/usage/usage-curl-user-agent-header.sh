#!/usr/bin/env bash
# @testcase: usage-curl-user-agent-header
# @title: curl sends user agent
# @description: Sends a custom User-Agent header with curl and verifies the server receives it.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-user-agent-header"
tmpdir=$(mktemp -d)
