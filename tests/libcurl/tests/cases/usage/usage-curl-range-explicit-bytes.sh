#!/usr/bin/env bash
# @testcase: usage-curl-range-explicit-bytes
# @title: curl range explicit bytes
# @description: Issues an explicit bytes range request through curl -r and verifies the partial body matches the requested slice.
# @timeout: 180
...
case_id="usage-curl-range-explicit-bytes"
tmpdir=$(mktemp -d)
