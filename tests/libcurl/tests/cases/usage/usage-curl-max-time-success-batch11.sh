#!/usr/bin/env bash
# @testcase: usage-curl-max-time-success-batch11
# @title: curl max time success
# @description: Completes a loopback transfer while curl --max-time is active.
# @timeout: 180
...
case_id="usage-curl-max-time-success-batch11"
tmpdir=$(mktemp -d)
