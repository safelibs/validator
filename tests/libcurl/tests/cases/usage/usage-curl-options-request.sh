#!/usr/bin/env bash
# @testcase: usage-curl-options-request
# @title: curl OPTIONS request
# @description: Requests HTTP OPTIONS with curl and checks the Allow header returned by the server.
# @timeout: 180
...
case_id="usage-curl-options-request"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
