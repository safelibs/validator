#!/usr/bin/env bash
# @testcase: usage-curl-form-post
# @title: curl posts multipart form
# @description: Sends multipart form data with curl and verifies the local server receives the field.
# @timeout: 180
# @tags: usage, http, upload
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-form-post"
tmpdir=$(mktemp -d)
