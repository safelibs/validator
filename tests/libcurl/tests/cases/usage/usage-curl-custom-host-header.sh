#!/usr/bin/env bash
# @testcase: usage-curl-custom-host-header
# @title: curl custom host header
# @description: Overrides the Host header through curl -H and verifies the server echoes the supplied authority value.
# @timeout: 180
...
case_id="usage-curl-custom-host-header"
tmpdir=$(mktemp -d)
