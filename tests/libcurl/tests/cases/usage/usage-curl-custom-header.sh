#!/usr/bin/env bash
# @testcase: usage-curl-custom-header
# @title: curl sends custom header
# @description: Sends a custom request header to a local HTTP handler and verifies it is received.
# @timeout: 120
# @tags: usage, http
# @client: curl
...
case_id="usage-curl-custom-header"
tmpdir=$(mktemp -d)
