#!/usr/bin/env bash
# @testcase: usage-curl-compressed-gzip-response-batch11
# @title: curl compressed gzip response
# @description: Downloads and decompresses a gzip-encoded HTTP response with curl.
# @timeout: 180
...
case_id="usage-curl-compressed-gzip-response-batch11"
tmpdir=$(mktemp -d)
