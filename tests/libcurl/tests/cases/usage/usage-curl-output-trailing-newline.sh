#!/usr/bin/env bash
# @testcase: usage-curl-output-trailing-newline
# @title: curl output file body
# @description: Downloads a response with curl -o and verifies the saved body file exists and contains the expected payload string.
# @timeout: 180
# @tags: usage, curl, output
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-output-trailing-newline"
tmpdir=$(mktemp -d)
