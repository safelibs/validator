#!/usr/bin/env bash
# @testcase: usage-curl-writeout-size
# @title: curl write-out size
# @description: Uses curl write-out variables to report the downloaded byte count for a response body.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-writeout-size"
tmpdir=$(mktemp -d)
