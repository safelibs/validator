#!/usr/bin/env bash
# @testcase: usage-curl-stderr-verbose-info
# @title: curl verbose stderr trace
# @description: Runs curl with -v and verifies the verbose stderr stream contains both the request line and the response status header.
# @timeout: 180
# @tags: usage, curl, debug
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-stderr-verbose-info"
tmpdir=$(mktemp -d)
