#!/usr/bin/env bash
# @testcase: usage-curl-interface-loopback
# @title: curl interface loopback
# @description: Binds a curl request to the loopback interface and verifies the local server response.
# @timeout: 180
...
case_id="usage-curl-interface-loopback"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
