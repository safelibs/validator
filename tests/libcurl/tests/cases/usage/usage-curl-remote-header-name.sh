#!/usr/bin/env bash
# @testcase: usage-curl-remote-header-name
# @title: curl remote header name
# @description: Downloads a response with Content-Disposition and verifies curl saves the server-provided filename.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-remote-header-name"
tmpdir=$(mktemp -d)
