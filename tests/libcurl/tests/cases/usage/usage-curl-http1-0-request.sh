#!/usr/bin/env bash
# @testcase: usage-curl-http1-0-request
# @title: curl HTTP/1.0 request
# @description: Forces an HTTP/1.0 request with curl and verifies the request version observed by the server.
# @timeout: 180
...
case_id="usage-curl-http1-0-request"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
