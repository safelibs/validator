#!/usr/bin/env bash
# @testcase: usage-curl-netrc-auth-header-batch11
# @title: curl netrc auth header
# @description: Sends HTTP basic credentials loaded from a netrc file with curl.
# @timeout: 180
...
case_id="usage-curl-netrc-auth-header-batch11"
tmpdir=$(mktemp -d)
