#!/usr/bin/env bash
# @testcase: usage-curl-range-request
# @title: curl range request
# @description: Requests a byte range from a local HTTP handler and checks the partial body.
# @timeout: 180
# @tags: usage, http
# @client: curl
...
case_id="usage-curl-range-request"
tmpdir=$(mktemp -d)
