#!/usr/bin/env bash
# @testcase: usage-curl-delete-request
# @title: curl DELETE request
# @description: Sends an HTTP DELETE request with curl and verifies the custom server acknowledges the method.
# @timeout: 180
# @tags: usage, http, cli
# @client: curl
...
case_id="usage-curl-delete-request"
tmpdir=$(mktemp -d)
