#!/usr/bin/env bash
# @testcase: usage-curl-compressed-response
# @title: curl decompresses HTTP body
# @description: Fetches a gzip-encoded local response with curl --compressed and verifies the decoded body.
# @timeout: 180
# @tags: usage, http, compression
# @client: curl

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-curl-compressed-response"
tmpdir=$(mktemp -d)
