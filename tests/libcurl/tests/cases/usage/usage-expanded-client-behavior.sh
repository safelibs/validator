...
case "$case_id" in
  usage-curl-head-follow-redirect)
    port=18101
    start_custom_server "$port"
    curl -fsSIL "http://127.0.0.1:$port/redirect" >"$tmpdir/out"
    validator_assert_contains "$tmpdir/out" '200 OK'
    ;;
  usage-curl-dump-headers-file)
    port=18102
    start_custom_server "$port"
    ...
    ;;
  usage-curl-data-urlencoded-post)
    port=18103
    start_custom_server "$port"
    curl -fsS --data-urlencode 'message=alpha beta' "http://127.0.0.1:$port/post" >"$tmpdir/out"
    validator_assert_contains "$tmpdir/out" 'message=alpha+beta'
    ;;
  usage-curl-cookie-jar-save)
    port=18104
    start_custom_server "$port"
    curl -fsS -c "$tmpdir/cookies.txt" "http://127.0.0.1:$port/set-cookie" >/dev/null
    validator_assert_contains "$tmpdir/cookies.txt" 'validator'
    ;;
  usage-curl-cookie-jar-send)
    port=18105
    start_custom_server "$port"
    curl -fsS -c "$tmpdir/cookies.txt" "http://127.0.0.1:$port/set-cookie" >/dev/null
    curl -fsS -b "$tmpdir/cookies.txt" "http://127.0.0.1:$port/echo-cookie" >"$tmpdir/out"
    validator_assert_contains "$tmpdir/out" 'cookie=validator=present'
    ;;
  usage-curl-basic-auth-header)
    port=18106
    start_custom_server "$port"
  ...
  usage-curl-form-post-multipart)
    port=18108
    start_custom_server "$port"
    curl -fsS -F 'name=validator' "http://127.0.0.1:$port/post" >"$tmpdir/out"
    validator_assert_contains "$tmpdir/out" 'multipart/form-data'
    validator_assert_contains "$tmpdir/out" 'validator'
    ;;
  usage-curl-get-query-string)
    port=18109
    start_custom_server "$port"
    curl -fsSG --data-urlencode 'name=alpha beta' "http://127.0.0.1:$port/query" >"$tmpdir/out"
    validator_assert_contains "$tmpdir/out" 'name=alpha+beta'
    ;;
  usage-curl-if-none-match-header)
    port=18110
    start_custom_server "$port"
    curl -fsSI -H 'If-None-Match: "validator-etag"' "http://127.0.0.1:$port/etag" >"$tmpdir/out"
    validator_assert_contains "$tmpdir/out" '304 Not Modified'
    ;;
  *)
    printf 'unknown libcurl expanded usage case: %s\n' "$case_id" >&2
    exit 2
    ;;
esac
