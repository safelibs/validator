...
use crate::types::{
    yaml_emitter_t,
    yaml_encoding_t::{YAML_ANY_ENCODING, YAML_UTF16LE_ENCODING, YAML_UTF8_ENCODING},
    yaml_error_type_t::YAML_WRITER_ERROR,
};
use crate::yaml::size_t;
use crate::{libc, PointerExt};
...
pub(crate) unsafe fn yaml_emitter_flush_impl(emitter: *mut yaml_emitter_t) -> Success {
...
pub unsafe extern "C" fn yaml_emitter_flush(emitter: *mut yaml_emitter_t) -> c_int {
    ffi::int_boundary(|| unsafe {
        if emitter.is_null()
            ...
            || (*emitter).encoding == YAML_ANY_ENCODING
        {
            return crate::FAIL;
        }

        if yaml_emitter_flush_impl(emitter).ok {
            crate::OK
        } else {
            crate::FAIL
        }
