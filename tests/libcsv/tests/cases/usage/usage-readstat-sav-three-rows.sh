#!/usr/bin/env bash
# @testcase: usage-readstat-sav-three-rows
# @title: readstat SAV three rows
# @description: Writes three CSV rows to SAV with readstat and verifies all rows return.
# @timeout: 180
# @tags: usage, csv
# @client: readstat
...
case_id="usage-readstat-sav-three-rows"
tmpdir=$(mktemp -d)
