#!/usr/bin/env bash
# @testcase: usage-readstat-dta-row-order
# @title: readstat DTA row order
# @description: Converts multiple CSV rows through DTA and verifies row ordering is preserved in exported data.
# @timeout: 180
# @tags: usage, csv
# @client: readstat
...
case_id="usage-readstat-dta-row-order"
tmpdir=$(mktemp -d)
