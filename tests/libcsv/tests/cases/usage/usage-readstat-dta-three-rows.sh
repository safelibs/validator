#!/usr/bin/env bash
# @testcase: usage-readstat-dta-three-rows
# @title: readstat DTA three rows
# @description: Writes three CSV rows to DTA with readstat and verifies all rows return.
# @timeout: 180
# @tags: usage, csv
# @client: readstat
...
case_id="usage-readstat-dta-three-rows"
tmpdir=$(mktemp -d)
