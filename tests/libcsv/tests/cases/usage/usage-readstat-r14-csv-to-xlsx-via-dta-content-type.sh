...
# Every OOXML package must contain a [Content_Types].xml part at the package
# root. Use python3 zipfile (always available in the libcsv container) rather
# than unzip(1), which the container does not include.
python3 - "$tmpdir/out.xlsx" <<'PY'
import sys, zipfile
with zipfile.ZipFile(sys.argv[1]) as zf:
    names = zf.namelist()
assert '[Content_Types].xml' in names, f'missing [Content_Types].xml; saw: {names[:8]}'
PY
