...
# @title: giffix on treescap.gif preserves the logical screen size
# @description: Runs giffix over treescap.gif (non-interlaced; giffix refuses interlaced inputs with "Cannot fix interlaced images.") and confirms the output is still recognised as a GIF and reports the same screen size via giftool -f '%s\n' as the input, exercising giffix's clean-pass-through path.
# @timeout: 60
