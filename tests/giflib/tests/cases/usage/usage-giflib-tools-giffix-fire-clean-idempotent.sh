...
# @title: giffix on already-clean treescap.gif preserves giftext metadata
# @description: Pipes the already-well-formed treescap.gif fixture through giffix and asserts the giftext header decoding (screen size, GIF version) of the giffix output matches the source, demonstrating giffix is a no-op-equivalent on healthy non-interlaced inputs.
# @timeout: 60
...
[[ "$orig_frames" == "$fixed_frames" && "$orig_frames" -ge 1 ]] || {
  printf 'frame count diverged: orig=%s fixed=%s\n' \
