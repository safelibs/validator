...
# No other disposal mode should remain. Use awk so an empty match does not
# trip pipefail (grep -v with no hits exits 1, which previously broke the
# script under set -o pipefail).
other=$(awk '/Disposal Mode:/ && !/Disposal Mode: 2$/' "$tmpdir/ext.txt" | wc -l)
if (( other > 0 )); then
  printf 'unexpected non-2 disposal modes after -x 2:\n' >&2
  awk '/Disposal Mode:/ && !/Disposal Mode: 2$/' "$tmpdir/ext.txt" >&2
  exit 1
fi
