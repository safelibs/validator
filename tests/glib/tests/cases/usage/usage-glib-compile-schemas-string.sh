#!/usr/bin/env bash
# @testcase: usage-glib-compile-schemas-string
# @title: gsettings string schema
# @description: Compiles a schema with a string key and reads it through gsettings.
# @timeout: 180
...
case_id="usage-glib-compile-schemas-string"
tmpdir=$(mktemp -d)
