...
#[test]
fn xchacha20_stream_vectors_match_upstream() {
    let tvs = [
        (
    ...
    for (key_hex, nonce_hex, out_hex) in tvs {
        let key = hex_decode(key_hex);
        let nonce = hex_decode(nonce_hex);
        let expected = hex_decode(out_hex);
        ...
        let mut zeros = vec![0u8; expected.len()];
        assert_eq!(
            xchacha20::crypto_stream_xchacha20_xor(
                zeros.as_mut_ptr(),
                expected.as_ptr(),
                expected.len() as u64,
                nonce.as_ptr(),
        ...
        assert_eq!(
            xchacha20::crypto_stream_xchacha20_xor_ic(
                zeros.as_mut_ptr(),
                expected.as_ptr(),
                expected.len() as u64,
                nonce.as_ptr(),
    ...
    let key = hex_decode(tvs[0].0);
    let nonce = hex_decode(tvs[0].1);
    let mut empty = Vec::<u8>::new();
    assert_eq!(
        xchacha20::crypto_stream_xchacha20(empty.as_mut_ptr(), 0, nonce.as_ptr(), key.as_ptr(),),
        0
    );
    assert_eq!(
        xchacha20::crypto_stream_xchacha20_xor(
    ...
    let mut block = (0u8..64).collect::<Vec<_>>();
    let mut doubled = (64u8..128).collect::<Vec<_>>();
    ...
    assert_eq!(&block[..], &doubled[64..]);

    let mut stitched = vec![0u8; 192];
    let mut single = vec![0u8; 192];
    assert_eq!(
        xchacha20::crypto_stream_xchacha20_xor_ic(
            single.as_mut_ptr(),
            single.as_ptr(),
    ...
    for (offset, ic) in [
        ...
        assert_eq!(
            xchacha20::crypto_stream_xchacha20_xor_ic(
                unsafe { stitched.as_mut_ptr().add(offset) },
                unsafe { stitched.as_ptr().add(offset) },
                64,
                nonce.as_ptr(),
                ic,
                key.as_ptr(),
    ...
    xchacha20::crypto_stream_xchacha20_keygen(generated_key.as_mut_ptr());
    assert!(generated_key.iter().any(|&byte| byte != 0));

    assert_eq!(xchacha20::crypto_stream_xchacha20_keybytes(), 32);
    assert_eq!(xchacha20::crypto_stream_xchacha20_noncebytes(), 24);
    assert!(xchacha20::crypto_stream_xchacha20_messagebytes_max() >= 192);
}

#[test]
fn xchacha20_secretbox_vectors_match_upstream() {
    let tvs = [
        (
        ...
        (
            ...
            "76bd706e07741e713d90efdb34ad202067263f984942aae8bda159f30dfccc72200f8093520b85c5ad124ff7c8b2d920946e5cfff4b819abf84c7b35a6205ca72c9f8747c3044dd73fb4bebda1b476",
        ...
        (
            ...
            "d5be1a24c7872115dc5c5b4234dbee35a6f89ae3a91b3e33d75249a0aecfed252341295f49296f7ee14d64de1ea6355cb8facd065052d869aeb1763cda7e418a7e33b6f7a81327181df6cd4de3a126d9df1b5e8b0b1a6b281e63f2",
        ...
        (
            ...
            "dbf165bb8352d6823991b99f3981ba9c8153635e5695477cba54e96a2a8c4dc5f9dbe817887d7340e3f48a",
    ...
    let macbytes = secretbox::crypto_secretbox_xchacha20poly1305_macbytes();

    for (key_hex, nonce_hex, msg_hex, out_hex) in tvs {
        let key = hex_decode(key_hex);
        let nonce = hex_decode(nonce_hex);
        ...
        let mut out = vec![0u8; expected.len()];
        assert_eq!(
            secretbox::crypto_secretbox_xchacha20poly1305_easy(
                out.as_mut_ptr(),
                msg.as_ptr(),
                0,
                nonce.as_ptr(),
        ...
        let opened_ptr = opened.as_mut_ptr();
        assert_eq!(
            secretbox::crypto_secretbox_xchacha20poly1305_open_easy(
                opened_ptr,
                opened_ptr.cast(),
                (macbytes - 1) as u64,
                nonce.as_ptr(),
        ...
        let tamper_index = expected.len() / 2;
        opened[tamper_index] ^= 1;
        assert_eq!(
            secretbox::crypto_secretbox_xchacha20poly1305_open_easy(
                opened_ptr,
                opened_ptr.cast(),
        ...
        let mut detached_out = vec![0u8; expected.len()];
        let mac_ptr = detached_out.as_mut_ptr();
        let cipher_ptr = unsafe { mac_ptr.add(macbytes) };
        assert_eq!(
            secretbox::crypto_secretbox_xchacha20poly1305_detached(
                cipher_ptr,
                mac_ptr,
                msg.as_ptr(),
                msg.len() as u64,
        ...
        assert_eq!(detached_out, expected);

        let mut plaintext = vec![0u8; msg.len()];
        assert_eq!(
            secretbox::crypto_secretbox_xchacha20poly1305_open_detached(
                plaintext.as_mut_ptr(),
                unsafe { expected.as_ptr().add(macbytes) },
                expected.as_ptr(),
                msg.len() as u64,
                nonce.as_ptr(),
                key.as_ptr(),
    ...
    assert_eq!(secretbox::crypto_secretbox_xchacha20poly1305_keybytes(), 32);
    assert_eq!(
        secretbox::crypto_secretbox_xchacha20poly1305_noncebytes(),
        24
    );
    assert_eq!(secretbox::crypto_secretbox_xchacha20poly1305_macbytes(), 16);
    assert!(secretbox::crypto_secretbox_xchacha20poly1305_messagebytes_max() >= 256);
}
