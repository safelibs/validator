...
# @description: Calls sodium::sig_keygen() to obtain an Ed25519 secret key, derives the public key with sig_pubkey(), signs a raw byte vector via sig_sign, asserts the signature is a 64-byte raw vector, then calls sig_verify with the public key and asserts the function returns TRUE for the untampered payload.
# @timeout: 60
...
sk <- sig_keygen()
stopifnot(is.raw(sk), length(sk) > 0)
pk <- sig_pubkey(sk)
