#!/usr/bin/env bash
# @testcase: usage-gpg-gen-random-length-batch11
# @title: gpg random length
# @description: Generates strong random bytes with gpg and checks the byte count.
# @timeout: 180
...
case_id="usage-gpg-gen-random-length-batch11"
tmpdir=$(mktemp -d)
