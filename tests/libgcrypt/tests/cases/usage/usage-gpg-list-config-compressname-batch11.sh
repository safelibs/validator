#!/usr/bin/env bash
# @testcase: usage-gpg-list-config-compressname-batch11
# @title: gpg list config compression names
# @description: Lists configured compression algorithm names with gpg and checks BZIP2 support is reported.
# @timeout: 180
...
case_id="usage-gpg-list-config-compressname-batch11"
tmpdir=$(mktemp -d)
