...
static void
content_foreach_func(ExifEntry *entry, void *UNUSED(callback_data))
{
	char buf[2000];
...
static void
data_foreach_func(ExifContent *content, void *callback_data)
{
...
static void
test_parse(const char *filename, void *callback_data, int swap)
{
	ExifData *d;
	int fd;
	unsigned char *data;
...
static void
split_ws_string(const char *string, test_parse_func func, void *callback_data)
{
...
static void
run_default_images(test_parse_func func, void *callback_data, int swap)
{
...
main(const int argc, const char *argv[])
{
	...
	if (argc > first) {
		...
		if (envar)
			...
			run_default_images(test_parse, callback_data, swap);
	}
