#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-protocol-field
# @title: iperf3 JSON UDP protocol field
# @description: Runs a short UDP iperf3 transfer and verifies the emitted JSON records the UDP protocol name.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-protocol-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
