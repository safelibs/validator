#!/usr/bin/env bash
# @testcase: usage-iperf3-json-reverse-received-bytes
# @title: iperf3 JSON reverse received bytes
# @description: Runs a reverse iperf3 test and verifies the received byte count reported in the JSON end section.
# @timeout: 180
...
case_id="usage-iperf3-json-reverse-received-bytes"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
