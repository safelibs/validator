#!/usr/bin/env bash
# @testcase: usage-iperf3-json-time-based
# @title: iperf3 JSON time based
# @description: Runs a short time-based iperf3 transfer and validates the resulting JSON payload.
# @timeout: 180
...
case_id="usage-iperf3-json-time-based"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
