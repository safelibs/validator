#!/usr/bin/env bash
# @testcase: usage-iperf3-json-parallel-three-streams-count
# @title: iperf3 JSON parallel three streams
# @description: Runs a three-stream iperf3 transfer and verifies the JSON metadata records the requested stream count.
# @timeout: 180
...
case_id="usage-iperf3-json-parallel-three-streams-count"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
