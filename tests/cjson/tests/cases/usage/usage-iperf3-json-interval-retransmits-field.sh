#!/usr/bin/env bash
# @testcase: usage-iperf3-json-interval-retransmits-field
# @title: iperf3 JSON retransmits field
# @description: Checks iperf3 JSON interval stream retransmit accounting exists for TCP.
# @timeout: 180
...
case_id="usage-iperf3-json-interval-retransmits-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
