#!/usr/bin/env bash
# @testcase: usage-iperf3-json-cport-paired-range-32007
# @title: iperf3 JSON paired ports range 32007
# @description: Runs iperf3 with a server port in the 32000-32099 range and a client port of 32007 so cjson must serialize both connected[0].remote_port and connected[0].local_port from a paired range distinct from any prior cport testcase.
# @timeout: 180
...
case_id="usage-iperf3-json-cport-paired-range-32007"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
...
jq -e --argjson rp "$server_port" --argjson lp "$client_port" \
  '.start.connected[0].remote_port == $rp and .start.connected[0].local_port == $lp and ($rp >= 32000 and $rp < 32100)' \
  "$tmpdir/client.json"
