#!/usr/bin/env bash
# @testcase: usage-iperf3-json-remote-port-field
# @title: iperf3 JSON remote port field
# @description: Exercises iperf3 json remote port field through a dependent-client usage scenario.
# @timeout: 180
# @tags: usage
# @client: iperf3
...
case_id="usage-iperf3-json-remote-port-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
