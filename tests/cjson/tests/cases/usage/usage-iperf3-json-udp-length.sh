#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-length
# @title: iperf3 JSON UDP packet length
# @description: Runs UDP iperf3 traffic with an explicit datagram length and validates the JSON report.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-length"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
