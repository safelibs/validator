#!/usr/bin/env bash
# @testcase: usage-iperf3-json-local-host-loopback
# @title: iperf3 JSON local host loopback
# @description: Verifies iperf3 JSON connection metadata reports the loopback local host during a client run.
# @timeout: 180
...
case_id="usage-iperf3-json-local-host-loopback"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
