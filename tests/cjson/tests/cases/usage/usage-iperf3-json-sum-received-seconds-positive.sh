#!/usr/bin/env bash
# @testcase: usage-iperf3-json-sum-received-seconds-positive
# @title: iperf3 JSON received duration
# @description: Checks iperf3 JSON reports a positive receive-side duration.
# @timeout: 180
...
case_id="usage-iperf3-json-sum-received-seconds-positive"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
