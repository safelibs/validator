#!/usr/bin/env bash
# @testcase: usage-iperf3-json-remote-host-loopback
# @title: iperf3 JSON remote host loopback
# @description: Verifies iperf3 JSON connection metadata reports the loopback remote host during a client run.
# @timeout: 180
...
case_id="usage-iperf3-json-remote-host-loopback"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
