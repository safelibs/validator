#!/usr/bin/env bash
# @testcase: usage-iperf3-json-tcp-protocol-field
# @title: iperf3 JSON TCP protocol field
# @description: Runs a short TCP iperf3 transfer and verifies the emitted JSON records the TCP protocol name.
# @timeout: 180
...
case_id="usage-iperf3-json-tcp-protocol-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
