#!/usr/bin/env bash
# @testcase: usage-iperf3-json-mss-clamp
# @title: iperf3 JSON MSS clamp
# @description: Runs iperf3 with a requested TCP MSS and checks JSON byte accounting.
# @timeout: 180
...
case_id="usage-iperf3-json-mss-clamp"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
