#!/usr/bin/env bash
# @testcase: usage-iperf3-json-parallel-stream-count
# @title: iperf3 JSON parallel stream count
# @description: Runs iperf3 with parallel client streams and verifies the JSON test metadata records the stream count.
# @timeout: 180
...
case_id="usage-iperf3-json-parallel-stream-count"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
