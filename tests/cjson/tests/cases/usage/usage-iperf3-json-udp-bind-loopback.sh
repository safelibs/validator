#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-bind-loopback
# @title: iperf3 JSON bound UDP client
# @description: Runs UDP iperf3 traffic from an explicit loopback bind address and checks JSON output.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-bind-loopback"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
