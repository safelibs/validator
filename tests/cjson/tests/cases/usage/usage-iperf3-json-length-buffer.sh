#!/usr/bin/env bash
# @testcase: usage-iperf3-json-length-buffer
# @title: iperf3 JSON buffer length
# @description: Runs iperf3 with a fixed buffer length and checks JSON sender totals.
# @timeout: 180
...
case_id="usage-iperf3-json-length-buffer"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
