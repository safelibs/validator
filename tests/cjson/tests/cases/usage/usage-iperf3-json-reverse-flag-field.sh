#!/usr/bin/env bash
# @testcase: usage-iperf3-json-reverse-flag-field
# @title: iperf3 JSON reverse flag field
# @description: Exercises iperf3 json reverse flag field through a dependent-client usage scenario.
# @timeout: 180
# @tags: usage
# @client: iperf3
...
case_id="usage-iperf3-json-reverse-flag-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
