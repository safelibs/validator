#!/usr/bin/env bash
# @testcase: usage-iperf3-json-blockcount
# @title: iperf3 JSON block count
# @description: Runs iperf3 with a fixed block count and verifies JSON transfer accounting.
# @timeout: 180
...
case_id="usage-iperf3-json-blockcount"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
