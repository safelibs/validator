#!/usr/bin/env bash
# @testcase: usage-iperf3-json-duration-two-seconds
# @title: iperf3 JSON duration two seconds
# @description: Runs a time-based iperf3 transfer and verifies the JSON metadata records a two-second duration.
# @timeout: 180
...
case_id="usage-iperf3-json-duration-two-seconds"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
