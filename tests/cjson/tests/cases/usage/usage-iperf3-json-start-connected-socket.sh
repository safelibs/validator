#!/usr/bin/env bash
# @testcase: usage-iperf3-json-start-connected-socket
# @title: iperf3 JSON connected socket
# @description: Checks iperf3 JSON exposes the connected socket descriptor from a loopback transfer.
# @timeout: 180
...
case_id="usage-iperf3-json-start-connected-socket"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
