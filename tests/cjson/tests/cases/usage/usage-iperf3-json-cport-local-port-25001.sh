#!/usr/bin/env bash
# @testcase: usage-iperf3-json-cport-local-port-25001
# @title: iperf3 JSON cport local port
# @description: Runs iperf3 with an explicit client port and verifies the recorded local port in the JSON connection metadata.
# @timeout: 180
...
case_id="usage-iperf3-json-cport-local-port-25001"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
