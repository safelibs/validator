#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-blksize-1400-field
# @title: iperf3 JSON UDP blksize 1400
# @description: Runs a UDP iperf3 transfer with a 1400-byte block size and verifies the JSON test metadata records the block size.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-blksize-1400-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
