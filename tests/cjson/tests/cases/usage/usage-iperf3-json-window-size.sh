#!/usr/bin/env bash
# @testcase: usage-iperf3-json-window-size
# @title: iperf3 JSON window sizing
# @description: Runs iperf3 with an explicit TCP window and checks JSON transfer totals.
# @timeout: 180
...
case_id="usage-iperf3-json-window-size"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
