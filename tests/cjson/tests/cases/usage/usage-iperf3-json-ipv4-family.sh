#!/usr/bin/env bash
# @testcase: usage-iperf3-json-ipv4-family
# @title: iperf3 JSON IPv4 client
# @description: Forces iperf3 to IPv4 loopback and validates that JSON throughput data is still produced.
# @timeout: 180
...
case_id="usage-iperf3-json-ipv4-family"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
