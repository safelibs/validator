#!/usr/bin/env bash
# @testcase: usage-iperf3-json-cookie-length-check
# @title: iperf3 JSON cookie length
# @description: Checks iperf3 JSON carries a nontrivial session cookie string from a loopback transfer.
# @timeout: 180
...
case_id="usage-iperf3-json-cookie-length-check"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
