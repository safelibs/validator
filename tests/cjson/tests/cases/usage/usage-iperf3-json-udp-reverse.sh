#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-reverse
# @title: iperf3 JSON reverse UDP
# @description: Runs a reverse UDP iperf3 transfer and verifies JSON UDP summaries.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-reverse"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
