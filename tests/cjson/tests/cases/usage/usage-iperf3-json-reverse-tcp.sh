#!/usr/bin/env bash
# @testcase: usage-iperf3-json-reverse-tcp
# @title: iperf3 JSON reverse TCP
# @description: Runs a reverse TCP iperf3 transfer and verifies the JSON transfer summary.
# @timeout: 180
...
case_id="usage-iperf3-json-reverse-tcp"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
