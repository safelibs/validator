#!/usr/bin/env bash
# @testcase: usage-iperf3-json-bind-loopback
# @title: iperf3 JSON bound client
# @description: Runs iperf3 from an explicit loopback bind address and validates JSON connection data.
# @timeout: 180
...
case_id="usage-iperf3-json-bind-loopback"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
