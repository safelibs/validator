#!/usr/bin/env bash
# @testcase: usage-iperf3-json-nodelay
# @title: iperf3 JSON no-delay transfer
# @description: Runs iperf3 with TCP no-delay enabled and verifies JSON throughput data.
# @timeout: 180
...
case_id="usage-iperf3-json-nodelay"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
