#!/usr/bin/env bash
# @testcase: usage-iperf3-json-connecting-to-host
# @title: iperf3 JSON connecting_to host
# @description: Verifies the cjson serializer encodes the connecting_to.host loopback target in the iperf3 JSON header.
# @timeout: 180
...
case_id="usage-iperf3-json-connecting-to-host"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
...
run_iperf_json_check '.start.connecting_to.host == "127.0.0.1" and .start.connecting_to.port == $expected_port' -n 16K
