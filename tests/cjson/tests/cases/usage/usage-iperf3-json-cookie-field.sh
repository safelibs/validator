#!/usr/bin/env bash
# @testcase: usage-iperf3-json-cookie-field
# @title: iperf3 JSON cookie field
# @description: Exercises iperf3 json cookie field through a dependent-client usage scenario.
# @timeout: 180
# @tags: usage
# @client: iperf3
...
case_id="usage-iperf3-json-cookie-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
