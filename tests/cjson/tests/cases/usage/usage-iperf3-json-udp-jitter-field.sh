#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-jitter-field
# @title: iperf3 JSON UDP jitter field
# @description: Runs a UDP iperf3 test and verifies the jitter field is present in the JSON end summary.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-jitter-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
