#!/usr/bin/env bash
# @testcase: usage-iperf3-json-omit-one-second-flag
# @title: iperf3 JSON omit one second
# @description: Runs an iperf3 transfer with a one-second omitted warmup interval and verifies the JSON omit setting.
# @timeout: 180
...
case_id="usage-iperf3-json-omit-one-second-flag"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
