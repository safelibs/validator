#!/usr/bin/env bash
# @testcase: usage-iperf3-json-sum-sent-seconds-positive
# @title: iperf3 JSON sent duration
# @description: Checks iperf3 JSON reports a positive sent-side duration.
# @timeout: 180
...
case_id="usage-iperf3-json-sum-sent-seconds-positive"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
