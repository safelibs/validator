#!/usr/bin/env bash
# @testcase: usage-iperf3-json-sum-received-sender-boolean
# @title: iperf3 JSON received sender flag
# @description: Checks iperf3 JSON emits the received summary sender flag as a boolean.
# @timeout: 180
...
case_id="usage-iperf3-json-sum-received-sender-boolean"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
