#!/usr/bin/env bash
# @testcase: usage-iperf3-json-sum-sent-sender-true
# @title: iperf3 JSON sent sender flag
# @description: Checks iperf3 JSON marks the sent summary as the sender side.
# @timeout: 180
...
case_id="usage-iperf3-json-sum-sent-sender-true"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
