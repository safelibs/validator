#!/usr/bin/env bash
# @testcase: usage-iperf3-json-timestamp-timesecs-positive
# @title: iperf3 JSON timestamp seconds
# @description: Checks iperf3 JSON timestamp seconds are populated for a loopback transfer.
# @timeout: 180
...
case_id="usage-iperf3-json-timestamp-timesecs-positive"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
