#!/usr/bin/env bash
# @testcase: usage-iperf3-json-parallel-tcp
# @title: iperf3 JSON parallel TCP
# @description: Runs parallel TCP streams with iperf3 and checks the emitted JSON summary.
# @timeout: 180
...
case_id="usage-iperf3-json-parallel-tcp"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
