#!/usr/bin/env bash
# @testcase: usage-iperf3-json-small-fixed
# @title: iperf3 JSON small fixed transfer
# @description: Runs a small fixed-size iperf3 transfer and checks JSON byte totals.
# @timeout: 180
...
case_id="usage-iperf3-json-small-fixed"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
