#!/usr/bin/env bash
# @testcase: usage-iperf3-json-end-stream-count
# @title: iperf3 JSON end stream count
# @description: Runs a parallel iperf3 test and verifies the number of stream summaries in the JSON end section.
# @timeout: 180
...
case_id="usage-iperf3-json-end-stream-count"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
