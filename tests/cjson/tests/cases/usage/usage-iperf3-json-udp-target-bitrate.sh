#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-target-bitrate
# @title: iperf3 JSON UDP target bitrate
# @description: Runs a UDP iperf3 test and verifies the JSON metadata records a target bitrate.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-target-bitrate"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
