#!/usr/bin/env bash
# @testcase: usage-iperf3-json-tos-field
# @title: iperf3 JSON TOS transfer
# @description: Runs iperf3 with a TOS value and validates JSON transfer fields.
# @timeout: 180
...
case_id="usage-iperf3-json-tos-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
