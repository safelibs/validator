#!/usr/bin/env bash
# @testcase: usage-iperf3-json-tos-hex-32
# @title: iperf3 JSON TOS 0x20 echo
# @description: Sets the IP TOS byte to 0x20 so the cjson serializer must echo the decimal value 32 in start.test_start.tos.
# @timeout: 180
...
case_id="usage-iperf3-json-tos-hex-32"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
...
run_iperf_json_check '(.start.test_start.tos // 0) == 32' -S 0x20 -n 16K
