#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-parallel
# @title: iperf3 JSON parallel UDP
# @description: Runs iperf3 UDP traffic with parallel streams and checks JSON interval data.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-parallel"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
