#!/usr/bin/env bash
# @testcase: usage-iperf3-json-connected-count
# @title: iperf3 JSON connected count
# @description: Exercises iperf3 json connected count through a dependent-client usage scenario.
# @timeout: 180
# @tags: usage
# @client: iperf3
...
case_id="usage-iperf3-json-connected-count"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
