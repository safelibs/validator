#!/usr/bin/env bash
# @testcase: usage-iperf3-json-udp-end-jitter-nonnegative
# @title: iperf3 JSON UDP jitter
# @description: Checks iperf3 JSON UDP summary reports nonnegative jitter.
# @timeout: 180
...
case_id="usage-iperf3-json-udp-end-jitter-nonnegative"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
