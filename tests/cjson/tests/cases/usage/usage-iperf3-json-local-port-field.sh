#!/usr/bin/env bash
# @testcase: usage-iperf3-json-local-port-field
# @title: iperf3 JSON local port field
# @description: Exercises iperf3 json local port field through a dependent-client usage scenario.
# @timeout: 180
# @tags: usage
# @client: iperf3
...
case_id="usage-iperf3-json-local-port-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
