#!/usr/bin/env bash
# @testcase: usage-iperf3-json-omit-second
# @title: iperf3 JSON omitted warmup second
# @description: Runs iperf3 with an omitted first second and verifies JSON totals remain available.
# @timeout: 180
...
case_id="usage-iperf3-json-omit-second"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
