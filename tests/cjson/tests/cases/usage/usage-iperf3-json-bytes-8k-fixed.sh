#!/usr/bin/env bash
# @testcase: usage-iperf3-json-bytes-8k-fixed
# @title: iperf3 JSON fixed 8K bytes
# @description: Runs a fixed-byte iperf3 test and verifies the requested byte count stored in the JSON metadata.
# @timeout: 180
...
case_id="usage-iperf3-json-bytes-8k-fixed"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
