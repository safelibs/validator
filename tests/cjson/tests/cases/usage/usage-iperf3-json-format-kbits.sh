#!/usr/bin/env bash
# @testcase: usage-iperf3-json-format-kbits
# @title: iperf3 JSON kbit format
# @description: Runs iperf3 with kilobit reporting selected and verifies JSON summaries remain numeric.
# @timeout: 180
...
case_id="usage-iperf3-json-format-kbits"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
