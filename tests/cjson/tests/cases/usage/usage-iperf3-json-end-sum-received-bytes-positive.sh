#!/usr/bin/env bash
# @testcase: usage-iperf3-json-end-sum-received-bytes-positive
# @title: iperf3 JSON end sum received bytes
# @description: Verifies end.sum_received.bytes in the iperf3 JSON summary is positive so cjson serializes the receiver-side byte accounting.
# @timeout: 180
...
case_id="usage-iperf3-json-end-sum-received-bytes-positive"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
