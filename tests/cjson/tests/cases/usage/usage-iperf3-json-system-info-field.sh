#!/usr/bin/env bash
# @testcase: usage-iperf3-json-system-info-field
# @title: iperf3 JSON system info field
# @description: Exercises iperf3 json system info field through a dependent-client usage scenario.
# @timeout: 180
# @tags: usage
# @client: iperf3
...
case_id="usage-iperf3-json-system-info-field"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
