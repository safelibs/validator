...
# @description: Saves a small RGB TIFF with a fixed description=... kwarg, reopens the file with Pillow, asserts tag_v2[270] equals the original description string, and asserts info['description'] surfaces the same string, exercising libtiff's ImageDescription tag write/read path.
# @timeout: 60
...
with Image.open(sys.argv[1]) as im:
    ...
    info_desc = im.info.get('description')
    assert info_desc == description, ('info', info_desc)
PY
