...
extern "C" {
    fn jpeg_resync_to_restart(cinfo: j_decompress_ptr, desired: ::core::ffi::c_int) -> boolean;
}
...
pub const JPEG_EOI: ::core::ffi::c_int = 0xd9 as ::core::ffi::c_int;
...
unsafe extern "C" fn fill_mem_input_buffer(mut cinfo: j_decompress_ptr) -> boolean {
    static mut mybuffer: [JOCTET; 4] = [
        0xff as ::core::ffi::c_int as JOCTET,
        JPEG_EOI as JOCTET,
        0 as ::core::ffi::c_int as JOCTET,
        0 as ::core::ffi::c_int as JOCTET,
    ];
    (*(*cinfo).err).msg_code = JWRN_JPEG_EOF as ::core::ffi::c_int;
    Some(
        (*(*cinfo).err)
            .emit_message
    ...
    (*(*cinfo).src).next_input_byte = &raw const mybuffer as *const JOCTET;
    (*(*cinfo).src).bytes_in_buffer = 2 as size_t;
    return TRUE;
}
unsafe extern "C" fn skip_input_data(
    mut cinfo: j_decompress_ptr,
    mut num_bytes: ::core::ffi::c_long,
) {
    let mut src: *mut jpeg_source_mgr = (*cinfo).src as *mut jpeg_source_mgr;
    if num_bytes > 0 as ::core::ffi::c_long {
        while num_bytes > (*src).bytes_in_buffer as ::core::ffi::c_long {
            num_bytes -= (*src).bytes_in_buffer as ::core::ffi::c_long;
        ...
        (*src).bytes_in_buffer = (*src).bytes_in_buffer.wrapping_sub(num_bytes as size_t);
    }
}
...
pub unsafe extern "C" fn jpeg_mem_src_tj(
    mut cinfo: j_decompress_ptr,
    mut inbuffer: *const ::core::ffi::c_uchar,
    mut insize: ::core::ffi::c_ulong,
) {
    let mut src: *mut jpeg_source_mgr = ::core::ptr::null_mut::<jpeg_source_mgr>();
    if inbuffer.is_null() || insize == 0 as ::core::ffi::c_ulong {
    ...
    if (*cinfo).src.is_null() {
        ...
        .expect("non-null function pointer")(
            ...
            JPOOL_PERMANENT,
            ::core::mem::size_of::<jpeg_source_mgr>() as size_t,
        ) as *mut jpeg_source_mgr as *mut jpeg_source_mgr;
    } else if (*(*cinfo).src).init_source
        != Some(init_mem_source as unsafe extern "C" fn(j_decompress_ptr) -> ())
    {
    ...
    src = (*cinfo).src as *mut jpeg_source_mgr;
    (*src).init_source = Some(init_mem_source as unsafe extern "C" fn(j_decompress_ptr) -> ())
        as Option<unsafe extern "C" fn(j_decompress_ptr) -> ()>;
    (*src).fill_input_buffer =
        Some(fill_mem_input_buffer as unsafe extern "C" fn(j_decompress_ptr) -> boolean)
            as Option<unsafe extern "C" fn(j_decompress_ptr) -> boolean>;
    (*src).skip_input_data =
        Some(skip_input_data as unsafe extern "C" fn(j_decompress_ptr, ::core::ffi::c_long) -> ())
            as Option<unsafe extern "C" fn(j_decompress_ptr, ::core::ffi::c_long) -> ()>;
    (*src).resync_to_restart = Some(
        jpeg_resync_to_restart
            as unsafe extern "C" fn(j_decompress_ptr, ::core::ffi::c_int) -> boolean,
    )
        as Option<unsafe extern "C" fn(j_decompress_ptr, ::core::ffi::c_int) -> boolean>;
    (*src).term_source = Some(term_source as unsafe extern "C" fn(j_decompress_ptr) -> ())
        as Option<unsafe extern "C" fn(j_decompress_ptr) -> ()>;
    (*src).bytes_in_buffer = insize as size_t;
