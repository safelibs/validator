...
unsafe extern "C" fn post_process_1pass(
    ...
    Some((*(*cinfo).upsample).upsample.expect("non-null function pointer"))
        .expect(
            "non-null function pointer",
        )(
        cinfo,
    ...
    Some((*(*cinfo).cquantize).color_quantize.expect("non-null function pointer"))
        .expect(
            "non-null function pointer",
        )(
        cinfo,
...
unsafe extern "C" fn post_process_prepass(
    ...
    if (*post).next_row == 0 as JDIMENSION {
        (*post).buffer = Some(
                (*(*cinfo).mem).access_virt_sarray.expect("non-null function pointer"),
            )
            .expect(
                "non-null function pointer",
            )(
...
pub unsafe extern "C" fn jinit_d_post_controller(
    ...
    let mut post: my_post_ptr = ::core::ptr::null_mut::<my_post_controller>();
    post = Some((*(*cinfo).mem).alloc_small.expect("non-null function pointer"))
        .expect(
            "non-null function pointer",
        )(
        cinfo as j_common_ptr,
        JPOOL_IMAGE,
        ::core::mem::size_of::<my_post_controller>() as size_t,
    ) as my_post_ptr;
    (*cinfo).post = post as *mut jpeg_d_post_controller as *mut jpeg_d_post_controller;
    (*post).pub_0.start_pass = Some(
        start_pass_dpost as unsafe extern "C" fn(j_decompress_ptr, J_BUF_MODE) -> (),
    ) as Option<unsafe extern "C" fn(j_decompress_ptr, J_BUF_MODE) -> ()>;
    (*post).whole_image = ::core::ptr::null_mut::<jvirt_sarray_control>();
    (*post).buffer = ::core::ptr::null_mut::<JSAMPROW>();
    if (*cinfo).quantize_colors != 0 {
