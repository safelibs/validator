...
case "$case_id" in
  usage-libarchive-tools-zstd-batch11-filelist-create)
    mkdir -p "$tmpdir/src"
    printf 'one\n' >"$tmpdir/src/one.txt"
    printf 'two\n' >"$tmpdir/src/two.txt"
    printf 'one.txt\ntwo.txt\n' >"$tmpdir/list"
    bsdtar -acf "$tmpdir/archive.tar.zst" -C "$tmpdir/src" -T "$tmpdir/list"
    ...
    ;;
  usage-libarchive-tools-zstd-batch11-null-filelist)
    mkdir -p "$tmpdir/src"
    printf 'alpha\n' >"$tmpdir/src/alpha.txt"
    printf 'beta\n' >"$tmpdir/src/beta.txt"
    printf 'alpha.txt\0beta.txt\0' >"$tmpdir/list0"
    bsdtar --null -acf "$tmpdir/archive.tar.zst" -C "$tmpdir/src" -T "$tmpdir/list0"
    bsdtar -tf "$tmpdir/archive.tar.zst" >"$tmpdir/out"
    validator_assert_contains "$tmpdir/out" 'alpha.txt'
    validator_assert_contains "$tmpdir/out" 'beta.txt'
    ;;
  usage-libarchive-tools-zstd-batch11-empty-directory)
    ...
    ;;
  usage-libarchive-tools-zstd-batch11-mode-preserved)
    mkdir -p "$tmpdir/src"
    printf '#!/bin/sh\n' >"$tmpdir/src/tool.sh"
    ...
    ;;
  usage-libarchive-tools-zstd-batch11-mtime-preserved)
    mkdir -p "$tmpdir/src"
    ...
    ;;
  usage-libarchive-tools-zstd-batch11-transform-name)
    mkdir -p "$tmpdir/src"
    ...
    ;;
  usage-libarchive-tools-zstd-batch11-newer-mtime)
    mkdir -p "$tmpdir/src"
    printf 'old\n' >"$tmpdir/src/old.txt"
    printf 'new\n' >"$tmpdir/src/new.txt"
    ...
    ;;
  usage-libarchive-tools-zstd-batch11-owner-labels)
    mkdir -p "$tmpdir/src"
    printf 'owner\n' >"$tmpdir/src/owned.txt"
    bsdtar --uid 123 --gid 456 --uname validator --gname validators -acf "$tmpdir/archive.tar.zst" -C "$tmpdir/src" owned.txt
    ...
    ;;
  usage-libarchive-tools-zstd-batch11-exclude-vcs)
    ...
    ;;
  usage-libarchive-tools-zstd-batch11-ustar-format)
    mkdir -p "$tmpdir/src"
