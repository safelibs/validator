use crate::{
    ...
    ffi::types::{ZDICT_cover_params_t, ZDICT_params_t, ZSTD_ErrorCode},
};
use core::ffi::{c_uint, c_void};

fn normalize_params(mut parameters: ZDICT_cover_params_t) -> ZDICT_cover_params_t {
    if parameters.k == 0 {
        parameters.k = 64;
    }
    if parameters.d == 0 {
...
fn zparams(parameters: ZDICT_cover_params_t) -> ZDICT_params_t {
    parameters.zParams
}
...
fn split_samples<'a>(
    ...
    let split = if split_point <= 0.0 { 1.0 } else { split_point }.clamp(0.0, 1.0);
