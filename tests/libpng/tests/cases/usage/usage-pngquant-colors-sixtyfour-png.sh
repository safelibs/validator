#!/usr/bin/env bash
# @testcase: usage-pngquant-colors-sixtyfour-png
# @title: pngquant --colors 64 palette cap
# @description: Quantises the basn2c08 PNG fixture to 64 colours and verifies the resulting palette stays at or below the cap.
# @timeout: 180
...
if len(colors) > 64:
    raise SystemExit(f'expected <=64 colours after --colors 64, got {len(colors)}')
if len(colors) < 2:
