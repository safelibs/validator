...
prepare_safe_packages() {
  ...
  for package in libpng16-16t64 libpng-dev libpng-tools; do
...
config = {
    ...
    "setup_notes": "The libpng host wrapper failed before launching ./test-original.sh.",
    "pre_marker_notes": (
        "The imported libpng harness failed before the first dependent marker; "
        "scratch-local package staging or image setup failed before dependent execution."
    ),
    "missing_marker_notes": (
        "The imported libpng harness exited successfully without emitting the full "
        "dependent marker sequence."
    ),
    "post_marker_failure_notes": "The imported libpng harness stopped after starting a dependent workload.",
}
...
PY
  ...
  if run_captured ./test-original.sh "${MODE}"; then
    status=0
  else
    status=$?
  fi
