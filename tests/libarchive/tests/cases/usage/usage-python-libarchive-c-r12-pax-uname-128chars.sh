...
# r5.2-1build1's python wrapper drops ArchiveEntry.uname; read it via the
# C accessor against the entry's internal pointer.
...
with libarchive.file_reader(str(arc)) as archive:
    for entry in archive:
        ptr = getattr(entry, "_entry_p", None) or getattr(entry, "_entry_pointer", None)
        if ptr is None:
            ptr = entry._entry_p  # raise if neither attr exists
        raw = get_uname(ptr)
        unames[entry.pathname] = raw.decode("ascii") if raw else None
