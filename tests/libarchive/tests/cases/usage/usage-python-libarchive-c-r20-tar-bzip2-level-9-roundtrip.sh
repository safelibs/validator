...
payload = (b"r20 bzip2 level-9 mem-writer " * 32) + bytes(range(96))
buf = io.BytesIO()
def cb(chunk):
    buf.write(bytes(chunk))
    return len(chunk)

with libarchive.custom_writer(cb, "ustar", "bzip2", options="compression-level=9") as writer:
...
print("bzip2-mem-level9-ok", len(payload))
PY
