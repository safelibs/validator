#!/usr/bin/env bash
# @testcase: usage-jshon-file-root-array
# @title: jshon file root array
# @description: Reads a root array from a file with jshon and verifies its length.
# @timeout: 180
# @tags: usage, json
# @client: jshon

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-jshon-file-root-array"
tmpdir=$(mktemp -d)
trap 'rm -rf "$tmpdir"' EXIT

json='{"name":"demo","count":7,"active":true,"disabled":false,"empty":null,"items":[1,2,3],"records":[{"name":"alpha"},{"name":"beta"}],"nested":{"value":"ok","label":"hello world"}}'
run_jshon() {
  printf '%s' "$1" | jshon "${@:2}" >"$tmpdir/out"
}

printf '[{"id":1},{"id":2},{"id":3}]' >"$tmpdir/input.json"
jshon -F "$tmpdir/input.json" -l >"$tmpdir/out"
validator_assert_contains "$tmpdir/out" '3'
