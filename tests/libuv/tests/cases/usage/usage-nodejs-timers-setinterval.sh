#!/usr/bin/env bash
# @testcase: usage-nodejs-timers-setinterval
# @title: Node.js timers setInterval
# @description: Runs repeated timer callbacks with setInterval and verifies the event loop executes the expected sequence.
# @timeout: 180
# @tags: usage, event-loop, timer
# @client: nodejs

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-nodejs-timers-setinterval"
tmpdir=$(mktemp -d)
