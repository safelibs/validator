#!/usr/bin/env bash
# @testcase: usage-nodejs-zlib-gunzip-buffer
# @title: Node.js zlib gunzip buffer
# @description: Gunzips an in-memory buffer through zlib and verifies the decompressed payload bytes.
# @timeout: 180
# @tags: usage, event-loop, compression
# @client: nodejs

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

case_id="usage-nodejs-zlib-gunzip-buffer"
tmpdir=$(mktemp -d)
