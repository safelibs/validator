...
prepare_safe_artifacts_manifest() {
  local runtime_deb
  local dev_deb
  local manifest_path="${HARNESS_ROOT}/safe/dist/artifacts.env"
  local runtime_name
  local dev_name

  if ! runtime_deb="$(find_exactly_one_deb "${HARNESS_ROOT}/safe/dist/libuv1t64_*.deb")"; then
    return 1
  fi
  if ! dev_deb="$(find_exactly_one_deb "${HARNESS_ROOT}/safe/dist/libuv1-dev_*.deb")"; then
    return 1
  fi

  runtime_name="$(basename "${runtime_deb}")"
  dev_name="$(basename "${dev_deb}")"
  printf 'LIBUV_SAFE_RUNTIME_DEB_REPO_REL=safe/dist/%s\n' "${runtime_name}" >"${manifest_path}"
  printf 'LIBUV_SAFE_DEV_DEB_REPO_REL=safe/dist/%s\n' "${dev_name}" >>"${manifest_path}"
}

build_config() {
  python3 - "${HARNESS_ROOT}/safe/tests/dependents/manifest.json" "${HARNESS_ROOT}/safe/tests/dependents" "${CONFIG_JSON}" <<'PY'
import json
import re
import sys
...
probes_root = Path(sys.argv[2])
selected = []
markers = []
pattern = re.compile(r'libuv_note "([^"]+)"')

for entry in manifest["dependents"]:
    selected.append(entry["id"])
    probe_path = probes_root / entry["probe_path"]
    marker = None
    for line in probe_path.read_text(encoding="utf-8").splitlines():
        ...
        if match:
            ...
            break
    if marker is None:
        raise SystemExit(f"missing libuv_note marker in {probe_path}")
    markers.append({"id": entry["id"], "marker": marker})
...
config = {
    ...
    "markers": markers,
    "setup_notes": "The libuv host wrapper failed before launching ./test-original.sh.",
    "pre_marker_notes": (
        "The imported libuv harness failed before the first dependent marker; "
        "dependency installation or packaged-libuv setup failed before the probe matrix began."
    ),
    "missing_marker_notes": (
        "The imported libuv harness exited successfully without emitting the full "
        "manifest-backed probe marker sequence."
    ),
    "post_marker_failure_notes": "The imported libuv harness stopped after starting a dependent workload.",
}
...
PY
  ...
  if [[ "${MODE}" == "safe" ]]; then
    export LIBUV_SAFE_DEB_DIR="${HARNESS_ROOT}/safe/dist"
    if prepare_safe_artifacts_manifest; then
      status=0
    else
      status=$?
      failure_mode="setup"
