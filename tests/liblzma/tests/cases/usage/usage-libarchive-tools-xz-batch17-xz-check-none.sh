#!/usr/bin/env bash
# @testcase: usage-libarchive-tools-xz-batch17-xz-check-none
# @title: xz --check=none disables checksum
# @description: Compresses a payload with xz --check=none, confirms xz --list reports a None integrity check, and round-trips through bsdtar to ensure liblzma accepts a checksum-free stream.
# @timeout: 180
# @tags: usage, archive, xz, cli, integrity
# @client: libarchive-tools

set -euo pipefail
source /validator/tests/_shared/runtime_helpers.sh

tmpdir=$(mktemp -d)
trap 'rm -rf "$tmpdir"' EXIT

mkdir -p "$tmpdir/src" "$tmpdir/out"
printf 'no-checksum payload\nrow two\n' >"$tmpdir/src/payload.txt"
src_sha=$(sha256sum "$tmpdir/src/payload.txt" | awk '{print $1}')

bsdtar -cf "$tmpdir/plain.tar" -C "$tmpdir/src" payload.txt
xz --check=none -z -c "$tmpdir/plain.tar" >"$tmpdir/plain.tar.xz"

# .xz magic
magic_hex=$(head -c 6 "$tmpdir/plain.tar.xz" | od -An -tx1 | tr -d ' \n')
test "$magic_hex" = "fd377a585a00"

# xz --list verbose must report the None check.
xz --list -vv "$tmpdir/plain.tar.xz" >"$tmpdir/list.txt" 2>&1
validator_assert_contains "$tmpdir/list.txt" 'None'

bsdtar -xf "$tmpdir/plain.tar.xz" -C "$tmpdir/out"
out_sha=$(sha256sum "$tmpdir/out/payload.txt" | awk '{print $1}')
[[ "$src_sha" == "$out_sha" ]] || {
  printf 'sha mismatch: %s vs %s\n' "$src_sha" "$out_sha" >&2
  exit 1
}
