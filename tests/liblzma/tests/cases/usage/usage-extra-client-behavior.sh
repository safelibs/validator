...
case "$case_id" in
  usage-libarchive-tools-xz-multi-file)
    make_tree
    ...
    ;;
  usage-libarchive-tools-xz-nested-extract)
    make_tree
    ...
    ;;
  usage-libarchive-tools-xz-strip-components)
    make_tree
    ...
    ;;
  usage-libarchive-tools-xz-stdin-archive)
    make_tree
    bsdtar -cJf - -C "$tmpdir/in" alpha.txt >"$tmpdir/stdin.tar.xz"
    bsdtar -tf "$tmpdir/stdin.tar.xz" | tee "$tmpdir/list"
    validator_assert_contains "$tmpdir/list" 'alpha.txt'
    ;;
  usage-libarchive-tools-xz-single-file-extract)
    make_tree
    bsdtar -cJf "$tmpdir/a.tar.xz" -C "$tmpdir/in" alpha.txt dir/beta.txt
    bsdtar -xf "$tmpdir/a.tar.xz" -C "$tmpdir/out" dir/beta.txt
    validator_assert_contains "$tmpdir/out/dir/beta.txt" 'beta payload'
    test ! -e "$tmpdir/out/alpha.txt"
    ;;
  usage-libarchive-tools-xz-exclude-pattern)
    make_tree
  ...
  usage-libarchive-tools-xz-checksum-compare)
    ...
    ;;
  usage-libarchive-tools-xz-verbose-list)
    make_tree
  ...
  usage-libarchive-tools-xz-filelist-input)
    ...
    validator_assert_contains "$tmpdir/list" 'dir/beta.txt'
    ;;
  usage-libarchive-tools-xz-extract-stdout)
    make_tree
    ...
    ;;
  usage-libarchive-tools-xz-dotfile)
    ...
    ;;
  usage-libarchive-tools-xz-leading-dash)
    mkdir -p "$tmpdir/in" "$tmpdir/out"
    printf 'dash payload\n' >"$tmpdir/in/-dash.txt"
    bsdtar -cJf "$tmpdir/a.tar.xz" -C "$tmpdir/in" -- '-dash.txt'
    bsdtar -xf "$tmpdir/a.tar.xz" -C "$tmpdir/out"
    validator_assert_contains "$tmpdir/out/-dash.txt" 'dash payload'
    ;;
  usage-libarchive-tools-xz-hardlink-pair)
    mkdir -p "$tmpdir/in" "$tmpdir/out"
    ...
    ;;
  usage-libarchive-tools-xz-subtree-only)
    make_tree
    ...
    ;;
  usage-libarchive-tools-xz-overwrite-existing)
    make_tree
    bsdtar -cJf "$tmpdir/a.tar.xz" -C "$tmpdir/in" alpha.txt
    mkdir -p "$tmpdir/out"
    ...
    ;;
  usage-libarchive-tools-xz-empty-file)
    mkdir -p "$tmpdir/in" "$tmpdir/out"
    : >"$tmpdir/in/empty.txt"
    ...
    ;;
  usage-libarchive-tools-xz-stream-list)
    make_tree
    bsdtar -cJf "$tmpdir/a.tar.xz" -C "$tmpdir/in" alpha.txt dir/sub/gamma.txt
    cat "$tmpdir/a.tar.xz" | bsdtar -tf - | tee "$tmpdir/list"
    validator_assert_contains "$tmpdir/list" 'alpha.txt'
    validator_assert_contains "$tmpdir/list" 'dir/sub/gamma.txt'
    ;;
  usage-libarchive-tools-xz-stream-extract)
    make_tree
    bsdtar -cJf "$tmpdir/a.tar.xz" -C "$tmpdir/in" alpha.txt
    mkdir -p "$tmpdir/out"
    ...
    ;;
  *)
    printf 'unknown liblzma extra usage case: %s\n' "$case_id" >&2
    exit 2
    ;;
esac
