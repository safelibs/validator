#!/usr/bin/env bash
# @testcase: usage-libarchive-tools-xz-batch14-options-preset4
# @title: bsdtar xz compression-level 4
# @description: Builds a tar.xz using --options xz:compression-level=4 (the libarchive-supported option name) and confirms .xz magic plus sha256 round-trip equality.
# @timeout: 180
...
for i in range(3000):
    sys.stdout.write("preset four content line %d\n" % i)' >"$tmpdir/in/payload.txt"
