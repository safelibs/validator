...
case "$case_id" in
  usage-libarchive-tools-xz-stdout-member-alpha)
    build_archive
    bsdtar -xOf "$archive" ./alpha.txt >"$tmpdir/out"
    validator_assert_contains "$tmpdir/out" 'alpha payload'
    ;;
  usage-libarchive-tools-xz-stdin-list-members)
    build_archive
  ...
  usage-libarchive-tools-xz-space-file-extract)
    ...
    ;;
  *)
    printf 'unknown liblzma expanded usage case: %s\n' "$case_id" >&2
    exit 2
    ;;
esac
