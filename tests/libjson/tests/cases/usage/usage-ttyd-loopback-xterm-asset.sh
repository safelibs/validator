#!/usr/bin/env bash
# @testcase: usage-ttyd-loopback-xterm-asset
# @title: ttyd loopback xterm asset
# @description: Starts ttyd on loopback and verifies the served index page references xterm assets.
# @timeout: 180
...
case_id="usage-ttyd-loopback-xterm-asset"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
