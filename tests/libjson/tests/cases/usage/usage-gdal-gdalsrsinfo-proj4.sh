#!/usr/bin/env bash
# @testcase: usage-gdal-gdalsrsinfo-proj4
# @title: gdal gdalsrsinfo proj4
# @description: Renders EPSG:4326 as PROJ.4 text with gdalsrsinfo and verifies the longlat projection token.
# @timeout: 180
# @tags: usage, gdal, srs
# @client: gdal
...
case_id="usage-gdal-gdalsrsinfo-proj4"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
