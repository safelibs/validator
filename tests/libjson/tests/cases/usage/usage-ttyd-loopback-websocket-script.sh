#!/usr/bin/env bash
# @testcase: usage-ttyd-loopback-websocket-script
# @title: ttyd loopback websocket path
# @description: Starts ttyd on loopback and verifies the served index page references the websocket endpoint.
# @timeout: 180
...
case_id="usage-ttyd-loopback-websocket-script"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
