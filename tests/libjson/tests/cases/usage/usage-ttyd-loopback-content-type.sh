#!/usr/bin/env bash
# @testcase: usage-ttyd-loopback-content-type
# @title: ttyd loopback content type
# @description: Starts ttyd on loopback and verifies the HTTP response content type header from the index page.
# @timeout: 180
...
case_id="usage-ttyd-loopback-content-type"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
