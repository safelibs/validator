#!/usr/bin/env bash
# @testcase: usage-gdal-ogrinfo-sql-average-value
# @title: gdal ogrinfo SQL average value
# @description: Runs an ogrinfo SQL aggregate query over GeoJSON and verifies the average value result.
# @timeout: 180
...
case_id="usage-gdal-ogrinfo-sql-average-value"
tmpdir=$(mktemp -d)
pid=""
cleanup() {
  if [[ -n "$pid" ]]; then
