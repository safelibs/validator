...
# @description: Builds an 8x4 image with constant value 1.7 cast to float, calls .round(:nearest), and asserts the result avg, min, and max are all exactly 2.0 (1.7 rounds to nearest integer 2), confirming libvips' :nearest rounding mode follows standard half-to-even/nearest semantics for fractional > 0.5.
# @timeout: 60
